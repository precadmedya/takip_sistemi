...
$custSql = "SELECT id, full_name FROM customers";
$prodSql = "SELECT id, name, price, currency, vat_rate FROM products";
$provSql = "SELECT id, name FROM providers";
if(!isAdmin()){ $filter = " WHERE user_id=".currentUserId(); $custSql .= $filter; $prodSql .= $filter; $provSql .= $filter; }
$customers = $pdo->query($custSql)->fetchAll(PDO::FETCH_ASSOC);
$products = $pdo->query($prodSql)->fetchAll(PDO::FETCH_ASSOC);
$providers = $pdo->query($provSql)->fetchAll(PDO::FETCH_ASSOC);
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $sql='UPDATE services SET customer_id=?, service_type=?, site_name=?, start_date=?, due_date=?, duration=?, price=?, currency=?, vat_rate=?, price_try=?, status=?, notes=?, reminder_enabled=?, reminder_days=? WHERE id=?';
    $params=[
        $_POST['customer_id'],
        $_POST['service_type'],
        $_POST['site_name'],
