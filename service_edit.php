...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $stmt = $pdo->prepare('UPDATE services SET customer_id=?, site_name=?, start_date=?, due_date=?, duration=?, price=?, currency=?, vat_rate=?, price_try=?, status=?, notes=?, reminder_enabled=?, reminder_days=? WHERE id=?');
    $stmt->execute([
        $_POST['customer_id'],
        $_POST['site_name'],
        $start,
