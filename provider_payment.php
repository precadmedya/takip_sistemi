...
$totPurch = (float)$pdo->query("SELECT SUM(price_try) FROM provider_purchases WHERE provider_id={$provider_id}$extra")->fetchColumn();
$totPay = (float)$pdo->query("SELECT SUM(amount_try) FROM provider_payments WHERE provider_id={$provider_id}$extra")->fetchColumn();
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $stmt = $pdo->prepare('INSERT INTO provider_payments(provider_id,amount_try,amount_orig,currency,pay_date,notes,user_id) VALUES (?,?,?,?,?,?,?)');
    $stmt->execute([$provider_id,$amount_try,$amount,$currency,$pay_date,$notes,currentUserId()]);
