...
$extra = isAdmin() ? '' : ' AND user_id='.currentUserId();
$totPurch = (float)$pdo->query("SELECT SUM(price_try) FROM provider_purchases WHERE provider_id={$id}$extra")->fetchColumn();
$totPay = (float)$pdo->query("SELECT SUM(amount_try) FROM provider_payments WHERE provider_id={$id}$extra")->fetchColumn();
...
$ins = $pdo->prepare('INSERT INTO provider_purchases(provider_id,item_name,quantity,unit,unit_price,vat_rate,currency,purchase_date,payment_date,price_try,notes,user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');
    foreach($_POST['item_name'] as $i=>$name){
        ...
        $ins->execute([$id,$name,$qty,$unit,$price,$vat,$cur,$pdate,$paydate,$try,$_POST['notes'][$i] ?? '',currentUserId()]);
    }
    $_SESSION['message']='Satın alım kaydedildi';
    header('Location: provider.php?id='.$id);
    exit;
}
$sql='SELECT * FROM provider_purchases WHERE provider_id=?';
$params=[$id];
if(!isAdmin()){ $sql.=' AND user_id=?'; $params[] = currentUserId(); }
$sql.=' ORDER BY purchase_date DESC';
$purchasesStmt = $pdo->prepare($sql);
$purchasesStmt->execute($params);
$purchases = $purchasesStmt->fetchAll(PDO::FETCH_ASSOC);
$sql='SELECT * FROM provider_payments WHERE provider_id=?';
$params=[$id];
if(!isAdmin()){ $sql.=' AND user_id=?'; $params[] = currentUserId(); }
