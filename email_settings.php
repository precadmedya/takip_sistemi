...
$editableFields = ['smtp_host','smtp_port','smtp_encryption','smtp_user','smtp_pass','smtp_from_name','smtp_from_email','smtp_timeout','smtp_reminder_copy'];
$settings=[];
$errors=[];
$save_message='';
$test_message='';
...
$editingEnabled = $postedSettings;
if(!empty($errors)){
    $editingEnabled = true;
}
if($save_message && empty($errors)){
    $editingEnabled = false;
}

$summary = [
    'Gönderen Adı' => $settings['smtp_from_name'],
    'Gönderen E-Posta' => $settings['smtp_from_email'],
    'Yönetici Kopya E-Postası' => $settings['smtp_reminder_copy'],
    'Sunucu' => $settings['smtp_host'],
    'Port' => $settings['smtp_port'],
    'Şifreleme' => strtoupper($settings['smtp_encryption']),
    'Kullanıcı Adı' => $settings['smtp_user'],
    'Zaman Aşımı (sn)' => $settings['smtp_timeout'] ?: 20,
];
...
<div class="card mb-4">
  <div class="card-header">Aktif Ayarlar</div>
  <ul class="list-group list-group-flush">
    <?php foreach($summary as $label=>$value): ?>
    <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap">
      <span class="fw-semibold me-2"><?= htmlspecialchars($label) ?></span>
      <span><?php if($value!=='' && $value!==null): ?><?= htmlspecialchars((string)$value) ?><?php else: ?><span class="text-muted">Tanımlı değil</span><?php endif; ?></span>
    </li>
    <?php endforeach; ?>
  </ul>
</div>
...
<form method="post" enctype="multipart/form-data">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="h5 mb-0">SMTP Ayarları</h2>
    <div class="d-flex gap-2">
      <button type="button" class="btn btn-outline-primary btn-sm <?= $editingEnabled ? 'd-none' : '' ?>" id="enable-edit">Düzenlemeyi Aktif Et</button>
      <button type="button" class="btn btn-outline-secondary btn-sm <?= $editingEnabled ? '' : 'd-none' ?>" id="cancel-edit">Düzenlemeyi Kapat</button>
    </div>
  </div>
  <fieldset id="smtp-settings" <?= $editingEnabled ? '' : 'disabled'; ?>>
  <div class="mb-3">
  ...
  <div class="d-flex justify-content-end gap-2 mt-3">
    <button type="submit" class="btn btn-primary" id="save-button" <?= $editingEnabled ? '' : 'disabled'; ?>>Kaydet</button>
  </div>
  </fieldset>
</form>
...
document.addEventListener('DOMContentLoaded',function(){
  const fieldset=document.getElementById('smtp-settings');
