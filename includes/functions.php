...
function getSmtpSettings(PDO $pdo): array {
    ...
    $settings = [];
    $stmt = $pdo->prepare("SELECT `key`, value FROM settings WHERE `key` IN ('smtp_host','smtp_port','smtp_encryption','smtp_user','smtp_pass')");
    $stmt->execute();
...
function sendMail(PDO $pdo, string $to, string $subject, string $body): bool {
    $smtp = getSmtpSettings($pdo);
    $headers = "From: {$smtp['username']}\r\n".
